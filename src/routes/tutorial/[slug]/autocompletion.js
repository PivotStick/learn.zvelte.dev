...
const options_for_svelte_events = zvelteEvents.map((event) =>
...
const options_for_svelte_attributes = snippet_for_attribute(zvelteAttributes);

const options_for_svelte_tags = zvelteTags.reduce((tags, tag) => {
...
function completion_for_attributes(context, node) {
	...
	const global_options = [...options_for_svelte_events, ...options_for_svelte_attributes];
...
export function autocomplete_for_zvelte() {
	return [
		// zvelteLanguage.data.of({
